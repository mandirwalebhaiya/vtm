...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const areaData = areas.find((a) => a.slug === area);       // ✅ no async in find
  const product = getProductBySlug(productSlug);

  if (!areaData || !product) return {};

  return {
    title: `${product.name} in ${areaData.name} | Vaishno Temple Delhi`,
    description: `Buy ${product.name} in ${areaData.name} Delhi. ${product.price}. ${product.description.slice(0, 120)}... Free home visit available.`,
    openGraph: {
    ...
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify({
          ...
          description: `Corian mandir manufacturer in ${areaLabel}, Delhi.`,
        ...
        <div className="grid md:grid-cols-2 gap-10 mb-12">
          ...
          <div className="flex flex-col gap-4">
            ...
            <h1 className="font-display text-3xl font-bold text-[#1a0e00] leading-snug">
              {product.name}{" "}
              <span className="text-[#c8621a]">in {areaData.name}, Delhi</span>
            </h1>
            ...
            <p className="text-[#8a7060] leading-relaxed text-sm">{product.description} we have beautifully installed this Corian Mandir in {areaData.name}.</p>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {otherProducts.map((p) => (
              <Link
                key={p.slug}
                href={`/corian-mandir-in-delhi/${areaData.slug}/${p.slug}`}
